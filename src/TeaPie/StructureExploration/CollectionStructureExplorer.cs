...
internal partial class CollectionStructureExplorer(ILogger<CollectionStructureExplorer> logger) : IStructureExplorer
{
    ...
    public IReadOnlyCollectionStructure Explore(ApplicationContext applicationContext)
    {
        ...
        var collectionStructure = ExploreCollection(applicationContext);

        LogEnd(collectionStructure.TestCases.Count);
    ...
    private static void CheckCollectionPath(string path)
    {
        ...
        {
            throw new InvalidOperationException($"Unable to explore collection on path '{path}' " +
